<script lang="ts">
	...
	async function openFile(node: FileNode) {
		...
		try {
			const res = await fetch(`${CODING_API}/fs/read?path=${encodeURIComponent(node.path)}`);
			if (res.ok) {
	...
	async function deleteFile(path: string) {
		try {
			const res = await fetch(`${CODING_API}/fs/delete?path=${encodeURIComponent(path)}`);
			if (res.ok) {
				if (activeFile === path) {
					activeFile = null;
					code = '';
...
<div class="flex h-screen w-full bg-[#0d1117] text-slate-300 font-sans overflow-hidden">
	...
	{#if globalState.isSidebarOpen}
		...
		>
			<div class="h-12 flex items-center justify-between px-4 border-b border-slate-800/30">
			...
			<div class="flex-1 overflow-y-auto py-2">
				{#each fileTree as node}
					<FileTreeItem {node} {expandedDirs} {activeFile} onopen={openFile} />
				{/each}
			</div>
		</aside>
	{/if}
